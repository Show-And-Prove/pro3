	...
	<section class="section">
		<div class="container">
			...
			<table class="table">
				...
				<tbody>
					<c:forEach items="${list }" var="dto" varStatus="status">
						<tr>
							<td>${status.count }</td>
							<td><c:set var="cate" value="${dto.cate }" /> <c:if test="${cate eq 'A' }">
									<span>관광</span>
								</c:if> <c:if test="${cate eq 'B' }">
									<span>체험</span>
								</c:if> <c:if test="${cate eq 'C' }">
									<span>행사</span>
								</c:if> <c:if test="${cate eq 'D' }">
									<span>축제</span>
								</c:if> <c:if test="${cate eq 'E' }">
									<span>숙박</span>
								</c:if> <c:if test="${cate eq 'F' }">
									<span>음식</span>
								</c:if> <c:if test="${cate eq 'G' }">
									<span>쇼핑</span>
								</c:if> <c:if test="${cate eq 'H' }">
									<span>기타</span>
								</c:if></td>
							<td><a href="${path1 }/GetTourDetailCtrl.do?no=${dto.no }">${dto.place }</a></td>
						</tr>
					</c:forEach>
			...
			<form name="frm1" id="frm1" class="search_form" action="${path1 }/GetPlaceSearchCtrl.do" method="get">
				...
				<input type="text" name="keyword" id="keyword" class="input inline" placeholder="검색어 입력" required> <input type="submit" class="button is-primary inline" value="검색">
			</form>
